classdef Camera < handle
    ...
    calibFile = fullfile('Lab5/calibrationSession.mat');

    % Check if calibration file exists
    if isfile(calibFile)

        load(calibFile, 'cameraParams', 'cam_R', 'cam_T', 'cam_IS', 'cam_pose');
        self.params = cameraParams;
        self.cam_R = cam_R;
        self.cam_T = cam_T;
        self.cam_IS = cam_IS;
        self.cam_pose = cam_pose;
        disp('[Camera] Loaded saved calibration.');
    else

        disp('[Camera] No calibration found. Running calibration...');
        self.params = self.calibrate();               % Run calibration
        [self.cam_IS, self.cam_pose] = self.calculateCameraPos(); % Compute extrinsics
        ...
        save(calibFile, 'cameraParams', 'cam_R', 'cam_T', 'cam_IS', 'cam_pose');
        disp('[Camera] Calibration complete and saved.');
    end
end
        ...
        function params = calibrate(self)
            ...
            try
                ...
                pause;
                disp("Calibrating");
                Camera_parameters; % Change this if you are using a different calibration script
                params = cameraParams;
